# pyController\ble_simple_central.py
...
def ble_connect():
    ...
    #接收信息处理 (包含对超声波文本的拦截处理)
    car_distance = "0.00"
    line_follower_active = False
    def on_rx(v):
        nonlocal car_distance, line_follower_active
        try:
            msg = v.decode('utf-8').strip()
            if msg.startswith("D:") or msg.startswith("DIST:"):
                parts = msg.split(',')
                for p in parts:
                    if p.startswith("D:") or p.startswith("DIST:"):
                        new_dist = p.split(":")[1]
                        if new_dist != car_distance:
                            car_distance = new_dist
                            # Padding to ensure there is no remnant text, and utilizing backcolor=WHITE
                            # avoids drawing an artificial screen-wiping background block.
                            display_text = f"{car_distance} cm"
                        ...
                        new_lf = (p.split(":")[1] == '1')
                        if new_lf != line_follower_active:
                            line_follower_active = new_lf
                            try:
                                if line_follower_active:
    ...
    while central.is_connected():
        ...
        # VERY IMPORTANT: Allow the Uploader to interrupt the script!
        except KeyboardInterrupt:
            raise 
            
        # Only catch generic errors so it doesn't mask the interrupt
